...
class EcocicloDonationController extends GetxController {
  EcocicloDonationController(this._donationRepository);
  ...
  void submitDonation(context) async {
    ...
    if (formKey.currentState!.validate() && comprovanteBytes().length > 2) {
      ...
      final response = await _donationRepository.postSendDonation(request, true);
      EcoLoading.hide();
      warningPhoto(false);
